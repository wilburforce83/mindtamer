...
class _SearchPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    void px(int x, int y) =>
        canvas.drawRect(Rect.fromLTWH(ox + x * u, oy + y * u, u, u), paint);
    // Circle approx
    for (final p in [
      [4, 3],
      [5, 3],
...
class _EditPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final oy = (size.height - u * 12) / 2;
    void px(int x, int y) =>
        canvas.drawRect(Rect.fromLTWH(ox + x * u, oy + y * u, u, u), p);
    // Pencil tip
    px(2, 9);
    px(3, 8);
    px(4, 7);
