...
class EchoWispIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final painter =
        CustomPaint(size: Size.square(size), painter: _WispPainter(c, v));
    if (!pixelate) return painter;
    return Pixelate(
      size: size,
      pixels: pixels,
      watch:
          '$seed:${color.toARGB32()}:${size.toStringAsFixed(2)}:$pixels:$pixelate',
      child: painter,
    );
  }
}
...
class _WispPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    tail.cubicTo(size.width * 0.10, radius * 1.2, -size.width * 0.08,
        radius * 1.5, 0, radius * 1.9);
  ...
  @override
  bool shouldRepaint(covariant _WispPainter oldDelegate) =>
