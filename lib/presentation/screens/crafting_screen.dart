...
class _CraftingScreenState extends State<CraftingScreen> {
  ...
  List<Echo> _loadEchoes() {
    try {
      ...
      return list.map((r){
        ...
        return Echo(id: r.echoId, element: el, rarity: rar, createdAt: r.createdAt, journalTitles: [r.title], entryId: r.entryId);
      }).toList();
    } catch (_) { return const <Echo>[]; }
  }

  ElementType _parseElement(String s) {
    final key = s.toLowerCase();
    for (final e in ElementType.values) { if (e.name == key) return e; }
    return ElementType.shadow;
  }
  Rarity _parseRarity(String s) {
    final key = s.toLowerCase();
    for (final r in Rarity.values) { if (r.name == key) return r; }
  ...
  Future<void> _forge() async {
    ...
    CraftedItem? result;
    if (_selItemA == null && _selItem == null && _selEchoA != null && _selEchoB != null) {
      result = await svc.craftArmorFromEchoes(_selEchoA!, _selEchoB!, playerClass: playerClass);
    } else if (_selItem != null && _selEchoA != null && _selItemA == null && _selEchoB == null) {
      result = await svc.upgradeWithEcho(_selItem!, _selEchoA!, playerClass: playerClass);
    } else if (_selItemA != null && _selItem != null && _selEchoA == null && _selEchoB == null) {
      result = await svc.craftFromItems(_selItemA!, _selItem!, playerClass: playerClass);
    }
    if (result != null) {
      // Consume inputs
      try {
      ...
      if (!mounted) return;
      await showDialog(context: context, builder: (_) => _ResultDialog(item: result!));
      // Clear selection after crafting
      setState((){ _selEchoA = null; _selEchoB = null; _selItemA = null; _selItem = null; });
    }
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    final bg = Image.asset('assets/images/crafting/crafting_background.png', fit: BoxFit.cover, filterQuality: FilterQuality.none,
      errorBuilder: (_, __, ___) => const SizedBox.shrink());
    final echoes = _loadEchoes();
    final items = CraftedInventoryService.all();

    final canForge = (!_busy) && (
      (_selItemA == null && _selItem == null && _selEchoA != null && _selEchoB != null) ||
      (_selItem != null && _selEchoA != null && _selItemA == null && _selEchoB == null) ||
      (_selItemA != null && _selItem != null && _selEchoA == null && _selEchoB == null)
    );
      ...
      body: Column(children: [
        ...
        Expanded(
          child: Padding(
            ...
            child: Row(children: [
            Expanded(child: _ListPanel(title: 'Resonant Echoes', children: [
              Padding(
                ...
                itemBuilder: (_, i) {
                  final e = echoes[i];
                  return InkWell(
                    onTap: (){ setState((){ _previewEcho = e; _previewItem = null; }); },
                    child: Container(
                      ...
                      child: EchoWispIcon(color: _echoColor(e), seed: e.id, size: 24, pixelate: true, pixels: 16),
                    ),
                  );
                },
              ),
            ...
            Expanded(child: _ListPanel(title: 'Weapons & Armor', children: [
              Padding(
                padding: const EdgeInsets.all(8),
                child: GridView.builder(
                shrinkWrap: true,
                ...
                itemBuilder: (_, i) {
                  ...
                  return InkWell(
                    onTap: (){ setState((){ _previewItem = it; _previewEcho = null; }); },
                    child: Container(
                      decoration: BoxDecoration(
                      ...
                      alignment: Alignment.center,
                      child: ItemIconBadge(iconPath: it.def.iconPath, rarity: it.rarity, element: it.element, tier: it.tier, size: 56, framed: false),
                    ),
                  );
                },
              ),
        ...
        _CraftDetailsPanel(
          echo: _previewEcho,
          item: _previewItem,
          onAddA: _previewEcho == null ? null : (){ setState((){ _selEchoA = _previewEcho; _previewEcho = null; }); },
          onAddB: _previewEcho == null ? null : (){ setState((){ _selEchoB = _previewEcho; _previewEcho = null; }); },
          onAddItemA: _previewItem == null ? null : (){ setState((){ _selItemA = _previewItem; _selEchoA = null; _previewItem = null; }); },
          onAddItemB: _previewItem == null ? null : (){ setState((){ _selItem = _previewItem; _selEchoB = null; _previewItem = null; }); },
        ),
...
class _ResultDialog extends StatelessWidget {
  final CraftedItem item; const _ResultDialog({required this.item});
  @override
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(child: ItemStatsLine(stats: item.stats, element: item.element)),
        ]),
        if (item.classAffinity != null) Text('Affinity: ${item.classAffinity}'),
      ]),
      actions: [
        TextButton(onPressed: ()=> Navigator.pop(context), child: const Text('Continue Crafting')),
      ],
    );
  }
}
...
class _CraftSlotBox extends StatelessWidget {
  final double size; final Widget child;
...
class _ForgeSideSlot extends StatelessWidget {
  final double size; final String title; final Widget child; final VoidCallback? onClear;
  const _ForgeSideSlot({required this.size, required this.title, required this.child, this.onClear});
  @override
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        if (onClear != null)
          SizedBox(
            width: size,
            child: TextButton(onPressed: onClear, child: const Text('Clear', style: TextStyle(fontSize: 11))),
          )
      ],
    );
  }
...
class _CraftDetailsPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
        if (echo != null) ...[
          Row(children: [
            EchoWispIcon(color: _colorForEcho(echo!), seed: echo!.id, size: 20, pixelate: true, pixels: 16),
            const SizedBox(width: 8),
            _pill(text: echo!.rarity.name.toUpperCase(), color: _rarityColor(echo!.rarity)),
            const SizedBox(width: 6),
            _pill(text: echo!.element.name, color: _elementColor(echo!.element)),
          ]),
          const SizedBox(height: 4),
          ...
          Align(
            ...
            child: Wrap(spacing: 8, children: [
              if (onAddA != null) OutlinedButton(onPressed: onAddA, child: const Text('Add to A', style: TextStyle(fontSize: 12))),
              if (onAddB != null) OutlinedButton(onPressed: onAddB, child: const Text('Add to B', style: TextStyle(fontSize: 12))),
            ]),
          )
        ],
        if (item != null) ...[
          ...
          if (item!.dnaJournalTitles.isNotEmpty) ...[
            ...
            const SizedBox(height: 2),
            ...item!.dnaJournalTitles.take(3).map((t) => Text('• $t', style: small)),
          ],
          const SizedBox(height: 6),
          Align(
            alignment: Alignment.centerRight,
            child: Wrap(spacing: 8, children: [
              if (onAddItemA != null) OutlinedButton(onPressed: onAddItemA, child: const Text('Add to A', style: TextStyle(fontSize: 12))),
              if (onAddItemB != null) OutlinedButton(onPressed: onAddItemB, child: const Text('Add to B', style: TextStyle(fontSize: 12))),
            ]),
          )
        ]
      ]),
  ...
  Color _elementColor(ElementType e) => _colorForEcho(Echo(id: '', element: e, rarity: Rarity.common, createdAt: DateTime.now(), journalTitles: const []));
